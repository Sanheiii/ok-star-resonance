...
class MinimapSectorAngleDetector:
    """识别小地图中半透明白色扇形的中心朝向。
    ...
    _MINIMAP_REGION = (71 / 2560, 47 / 1440, 295 / 2560, 271 / 1440)
    _ANGLE_BINS = 360
    ...
    _CENTER_EXCLUSION_RADIUS_RATIO = 15.0 / 1440.0
    ...
    def detect(cls, frame):
        """Crop a full frame and return detection data, or ``None`` if not found."""

        if frame is None or frame.size == 0:
            raise ValueError("Frame is empty.")
        frame_height, frame_width = frame.shape[:2]
        left, top, right, bottom = cls._MINIMAP_REGION
        minimap = frame[
            int(frame_height * top):int(frame_height * bottom),
            int(frame_width * left):int(frame_width * right),
        ]
        if minimap.size == 0:
            raise ValueError("Minimap region is empty.")
        center_exclusion_radius = frame_height * cls._CENTER_EXCLUSION_RADIUS_RATIO
        return cls._estimate_sector_angle(
            minimap,
            cls._SECTOR_WIDTH_DEGRESS,
    ...
    def _estimate_sector_angle(
        ...
        """Calculate angle and metrics, returning ``None`` if no sector is found."""
        ...
        if not edge_pairs:
            logger.debug("No complete bright/dark minimap sector edge pair was found")
            return None
